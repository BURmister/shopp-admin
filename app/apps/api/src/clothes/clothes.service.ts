...
export class ClothesService {
  ...
  async getAll(searchTerm?: string | Types.ObjectId) {
    ...
    const product = await this.ClothesModel.find().exec();
    return product;
  }

  async getById(_id: Types.ObjectId) {
    const product = await this.ClothesModel.findOne({ _id }, '-__v');
    if (!product) throw new UnauthorizedException('Product not found');

    return product;
  }

  async addOne(dto: ClothesDto) {
    const newProduct = new this.ClothesModel(dto);
    const product = await newProduct.save();
    return product._id;
  }

  async editOne(_id: Types.ObjectId, dto: ClothesDto) {
